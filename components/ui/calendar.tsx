	...
	return (
		<DayPicker
			...
			classNames={{
				...
				caption_dropdowns: "flex gap-1 flex-row-reverse",
				...classNames,
			}}
			components={{
				...
				Dropdown: dropdownProps => {
					...
					if (dropdownProps.name === "months") {
						selectValues = Array.from({ length: 12 }, (_, i) => {
							return {
								value: i.toString(),
								label: format(new Date(new Date().getFullYear(), i, 1), "M月"),
							};
						});
					...
					const caption = format(
						currentMonth,
						dropdownProps.name === "months" ? "M月" : "yyyy"
					);
						...
						<Select
							onValueChange={newValue => {
								if (dropdownProps.name === "years") {
									...
									goToMonth(newDate);
								} else if (dropdownProps.name === "months") {
									const newDate = parse(newValue, "M月", currentMonth);
									goToMonth(newDate);
								}
							}}
